...
if [[ $jpegxl = y ]] || { [[ $ffmpeg != no ]] && enabled libjxl; }; then
    ...
    if do_vcs "$SOURCE_REPO_LIBJXL"; then
        ...
        CXXFLAGS+=" -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DJXL_CMS_STATIC_DEFINE -DJXL_STATIC_DEFINE -DJXL_THREADS_STATIC_DEFINE" \
...
if [[ $ffmpeg != no ]] && enabled_any frei0r ladspa; then
    ...
    if do_vcs "$SOURCE_REPO_FREI0R"; then
        ...
        do_cmakeinstall -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DWITHOUT_OPENCV=on
        do_checkIfExist
    fi
fi
    ...
    if do_vcs "$SOURCE_REPO_X264" ||
        ...
        if [[ $standalone = y || $av1an != n ]] && [[ $x264 =~ (full|fullv) ]]; then
            ...
            if do_vcs "$SOURCE_REPO_FFMS2"; then
                ...
                do_patch "https://raw.githubusercontent.com/sgscaffidi3/mabs-patches/refs/heads/master/ffms2/ffms2_fix_linking.patch" am
                mkdir -p src/config
                do_autoreconf
